    ...
    def general_passing(self,text:Dict[str]):
        ...
        if result in tasks:
            ...
            if result == "create social media captions from image":
                return gpt3.generateCaption(text['link'])
            elif result == "correct grammar":
                return gpt3.correctGrammar(text['query'])
            elif result == "summarize for second grader":
                return gpt3.summarizeForSecondGrader(text['query'],30)
            elif result == "extract keywords":
                return gpt3.extractKeywords(text['query'])
            elif result == "to the point answere generation":
                return gpt3.factual_answere_generation(text['query'])
            elif result == "generate product name":
                return gpt3.name_generator(text['query'])
            elif result == "extract name and mailing address":
                return gpt3.extract_name_mailing(text['query'])
            elif result == "create ad copy":
                return gpt3.ad_copy_creation(text['query'])
            else:
                return gpt3.query(text['query'])
            ...
            if res=="Conversation with a bot":
                return gpt3Conversation.conversation_with_bot(text['query'])
            else:
                return gpt3Conversation.emulate_a_friend(text['query'])
        else:
            StableDiffusion().make_image(text['query'])
            return res
