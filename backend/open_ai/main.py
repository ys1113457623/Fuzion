...
if __name__ == "__main__":
    ...
    print(DT.correctGrammar("We can see that the step count is least for month 8 which corresponds to september which is expected because we only have data till 12th of September. We acn also observe that there is a peak for step count in may, and august. We can see that there was a sudden drop in step count from may to june, which can indicate some change in weather or environment which might lead to a drop in physical activity levels or it can be due to health issues, we can confirm that after looking at data from more people."))
